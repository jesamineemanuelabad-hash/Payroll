...
import {
  ...
  CheckCircle2, CircleAlert, Clock3, Coins, Download, FileSpreadsheet, HeartPulse, MapPin,
...
function duration(minutes: number) {
  ...
  return hours ? `${hours.toLocaleString()}h ${remainder}m` : `${remainder}m`;
}
...
export function HrAnalyticsDashboard({ initialData }: { initialData: LiveDashboardData }) {
  ...
  const rows:ExportRecord[]=[
    ...
    {metric:"Employees missing salary",value:data.accuracy.coverage.employeesMissingSalary,unit:"Employees",period:"Effective today",source:"Compensation history"},
    {metric:"Incomplete attendance punches",value:data.accuracy.coverage.incompleteAttendanceRecords,unit:"Records",period:`Last ${filters.months} months`,source:"Attendance ledger"},
    {metric:"Late minutes",value:data.accuracy.timeTotals.lateMinutes,unit:"Minutes",period:`Last ${filters.months} months`,source:"Attendance ledger"},
    {metric:"Absence minutes",value:data.accuracy.timeTotals.absenceMinutes,unit:"Minutes",period:`Last ${filters.months} months`,source:"Attendance ledger"},
    {metric:"SSS employee deductions",value:data.accuracy.payroll?.sssEmployee??0,unit:"PHP",period,source:"Latest payroll ledger"},
    {metric:"PhilHealth employee deductions",value:data.accuracy.payroll?.philhealthEmployee??0,unit:"PHP",period,source:"Latest payroll ledger"},
    {metric:"Pag-IBIG employee deductions",value:data.accuracy.payroll?.pagibigEmployee??0,unit:"PHP",period,source:"Latest payroll ledger"},
    {metric:"Late deductions",value:data.accuracy.payroll?.lateDeduction??0,unit:"PHP",period,source:"Latest payroll ledger"},
    {metric:"Absence deductions",value:data.accuracy.payroll?.absenceDeduction??0,unit:"PHP",period,source:"Latest payroll ledger"},
  ];
  ...
  const coverage=data.accuracy.coverage;
  const attendanceComplete=coverage.attendanceRecords?coverage.completeAttendanceRecords/coverage.attendanceRecords*100:0;
  const salaryComplete=coverage.employees?coverage.employeesWithSalary/coverage.employees*100:0;
  const hasAccuracyIssues=coverage.employeesMissingSalary>0||coverage.incompleteAttendanceRecords>0||!data.accuracy.payroll?.calculatedAt||(data.accuracy.payroll?.missingEmployeeEntries??0)>0;
  ...
  return <div aria-busy={pending||scoring}>
    ...
    <section aria-label="Payroll data accuracy" className={cn("mt-6 overflow-hidden rounded-xl border",hasAccuracyIssues?"border-amber-200 bg-amber-50/40":"border-emerald-200 bg-emerald-50/30")}><div className="flex flex-col gap-3 border-b border-inherit p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className={cn("grid size-9 shrink-0 place-items-center rounded-lg",hasAccuracyIssues?"bg-amber-100 text-amber-700":"bg-emerald-100 text-emerald-700")}>{hasAccuracyIssues?<CircleAlert className="size-4"/>:<CheckCircle2 className="size-4"/>}</span><div><h2 className="text-sm font-semibold text-slate-900">Live payroll data readiness</h2><p className="mt-1 text-xs leading-5 text-slate-600">Calculated only from saved employee salary history, actual attendance punches, and the latest payroll ledger.</p></div></div>{hasAccuracyIssues&&<Button variant="secondary" size="sm" asChild><Link href="/payroll-benefits/payroll">Complete payroll data<ChevronRight/></Link></Button>}</div><div className="grid bg-white sm:grid-cols-2 xl:grid-cols-4">{[
      {label:"Salary coverage",value:`${salaryComplete.toFixed(1)}%`,helper:`${coverage.employeesMissingSalary} of ${coverage.employees} employees missing salary`},
      {label:"Attendance completeness",value:`${attendanceComplete.toFixed(1)}%`,helper:`${coverage.incompleteAttendanceRecords} incomplete time-in/time-out records`},
      {label:"Employees with attendance",value:coverage.attendanceEmployees.toLocaleString(),helper:`${coverage.attendanceRecords.toLocaleString()} records in selected range`},
      {label:"Latest payroll entries",value:(data.accuracy.payroll?.employeeEntries??0).toLocaleString(),helper:(data.accuracy.payroll?.missingEmployeeEntries??0)>0?`${data.accuracy.payroll?.missingEmployeeEntries} salaried employees missing from this run`:data.accuracy.payroll?.calculatedAt?`Calculated ${new Date(data.accuracy.payroll.calculatedAt).toLocaleString()}`:"Payroll has not been calculated"},
    ].map((item)=><div className="border-b border-r p-4 last:border-r-0 sm:border-b-0" key={item.label}><p className="text-xs font-medium text-slate-500">{item.label}</p><p className="mt-2 text-xl font-semibold text-slate-950">{item.value}</p><p className="mt-1 text-xs text-slate-500">{item.helper}</p></div>)}</div></section>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-2"><section className="rounded-xl border border-slate-200 bg-white p-5"><ChartHeader title="Actual attendance totals" description={`Saved time-in/time-out computations for the last ${filters.months} months`} /><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">{[
      ["Worked",data.accuracy.timeTotals.workedMinutes],["Late",data.accuracy.timeTotals.lateMinutes],["Undertime",data.accuracy.timeTotals.undertimeMinutes],["Overtime",data.accuracy.timeTotals.overtimeMinutes],["Absence",data.accuracy.timeTotals.absenceMinutes],
    ].map(([label,value])=><div className="rounded-lg bg-slate-50 p-3" key={String(label)}><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-sm font-semibold text-slate-900">{duration(Number(value))}</p></div>)}</div></section><section className="rounded-xl border border-slate-200 bg-white p-5"><ChartHeader title="Itemized employee deductions" description={data.accuracy.payroll?`${data.accuracy.payroll.periodStart} to ${data.accuracy.payroll.periodEnd}`:"Calculate a payroll run to populate deductions"} />{data.accuracy.payroll?<div className="mt-5 grid grid-cols-2 gap-x-6 gap-y-3">{[
      ["SSS",data.accuracy.payroll.sssEmployee],["PhilHealth",data.accuracy.payroll.philhealthEmployee],["Pag-IBIG",data.accuracy.payroll.pagibigEmployee],["Withholding tax",data.accuracy.payroll.withholdingTax],["Late",data.accuracy.payroll.lateDeduction],["Undertime",data.accuracy.payroll.undertimeDeduction],["Absence",data.accuracy.payroll.absenceDeduction],["Other",data.accuracy.payroll.otherDeductions],
    ].map(([label,value])=><div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-2 text-sm" key={String(label)}><span className="text-slate-500">{label}</span><span className="font-medium text-slate-800">{formatCurrency(Number(value))}</span></div>)}<div className="col-span-2 flex items-center justify-between pt-1 text-sm font-semibold"><span>Total deductions</span><span>{formatCurrency(data.accuracy.payroll.totalDeductions)}</span></div></div>:<EmptyChart message="Import salaries and attendance, create a payroll period, then calculate the run."/>}</section></div>

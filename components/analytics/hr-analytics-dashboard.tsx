...
import {
  ...
  Play, RefreshCw, SlidersHorizontal, Sparkles, UsersRound, WalletCards,
} from "lucide-react";
import { toast } from "sonner";
import { loadLiveDashboard, runAttendanceScoring } from "@/app/actions/dashboard";
...
export function HrAnalyticsDashboard({ initialData }: { initialData: LiveDashboardData }) {
  ...
  function update(next:DashboardFilters,notify=false){const id=++requestId.current;setFilters(next);setError("");startTransition(async()=>{const result=await loadLiveDashboard(next);if(id!==requestId.current)return;if(!result.ok){setError(result.message);toast.error("Analytics refresh failed",{description:result.message});return;}setData(result.data);setAnimationKey((key)=>key+1);if(notify)toast.success("Live analytics refreshed");});}
  async function score(){setScoring(true);const result=await runAttendanceScoring();setScoring(false);if(!result.ok){toast.error("XGBoost scoring unavailable",{description:result.message});return;}toast.success("XGBoost scoring completed",{description:result.message});update(filters);}
  ...
  return <div aria-busy={pending||scoring}>
    ...
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start"><div><div className="flex items-center gap-2.5"><h1 className="text-2xl font-semibold tracking-[-0.03em] text-slate-950 sm:text-[28px]">HR Analytics</h1><span className="rounded-md bg-emerald-50 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-700">Live data</span></div><p className="mt-1.5 max-w-2xl text-sm leading-6 text-slate-500">Reporting calculated from saved payroll, employee, compensation, benefit, attendance, and model records.</p></div><div className="flex flex-wrap items-center gap-2"><Button variant="secondary" onClick={()=>{downloadRecordsAsCsv(`hr-analytics-${data.generatedAt.slice(0,10)}`,analyticsExportColumns,rows);toast.success("Live analytics downloaded");}}><Download />Download</Button><Button variant="secondary" onClick={exportExcel}><FileSpreadsheet />Export Excel</Button><Button variant="secondary" onClick={score} disabled={pending||scoring}><Play className={scoring?"animate-pulse":""} />{scoring?"Scoring…":"Run XGBoost"}</Button><Button onClick={()=>update(filters,true)} disabled={pending||scoring}><RefreshCw className={pending?"animate-spin":""} />{pending?"Refreshing…":"Refresh data"}</Button></div></div>

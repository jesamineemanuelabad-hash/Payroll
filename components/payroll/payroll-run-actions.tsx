...
import { BadgeCheck, Banknote, Calculator, FileSpreadsheet, Printer, RotateCcw, Send, ShieldCheck } from "lucide-react";
import { toast } from "sonner";
import { calculatePayrollRun, getPayrollRunReport, transitionPayrollRun, validatePayrollRun } from "@/app/actions/payroll";
...
const columns: ExportColumn[] = [
  ...
  { key: "nightMinutes", label: "Night Minutes", width: 14, format: "number" }, { key: "basicSalary", label: "Basic Pay", width: 16, format: "currency" },
  { key: "allowances", label: "Allowances", width: 16, format: "currency" }, { key: "overtimePay", label: "Overtime Pay", width: 16, format: "currency" },
  { key: "nightDifferential", label: "Night Differential", width: 18, format: "currency" }, { key: "bonus", label: "Bonus", width: 16, format: "currency" },
  ...
  { key: "pagibigEmployee", label: "Pag-IBIG", width: 14, format: "currency" }, { key: "withholdingTax", label: "BIR Withholding", width: 18, format: "currency" },
  { key: "otherDeductions", label: "Other Deductions", width: 18, format: "currency" }, { key: "totalDeductions", label: "Total Deductions", width: 18, format: "currency" },
  { key: "netPay", label: "Net Pay", width: 16, format: "currency" },
];
...
export function PayrollRunActions({ runId, status, calculatedAt, employeeCount, validationStatus, roles }: { runId: string; status: PayrollStatus; calculatedAt: string | null; employeeCount: number; validationStatus?: string; roles: string[] }) {
  ...
  function exportExcel() { startTransition(async () => { const result = await getPayrollRunReport(runId); if (!result.ok) { toast.error("Excel export failed", { description: result.message }); return; } const records = result.data.items.map((item) => ({ ...item }) as unknown as ExportRecord); await exportRecordsToExcel(`payroll-${result.data.run.period_start}-${result.data.run.period_end}`, "Payroll Register", columns, records); toast.success("Payroll register exported", { description: `${records.length} employee rows included.` }); }); }
